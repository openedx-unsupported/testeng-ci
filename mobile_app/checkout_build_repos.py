...
def _write_properties(
    ...
    """
    Writes an edx.properties file to the code repo
    that points that points at the proper location in the config
    repo.
...
def checkout_repos(build_environment_path, environ, base_path):
    ...
    code_repo_path = os.path.join(base_path, CODE_CHECKOUT)
    config_repo_path = os.path.join(base_path, CONFIG_CHECKOUT)
