...
class WebhookProcessorTestCase(TestCase):
    ...
    def test_get_target_urls(self):
        ...
        assert urls == ['http://www.example.com/webhooks']

    @patch.dict(os.environ, {'TARGET_URLS': 'http://www.a.com/foo,http://www.b.com/bar'})
    def test_get_target_urls(self):
        urls = _get_target_urls()
        assert urls == ['http://www.a.com/foo', 'http://www.b.com/bar']

    def test_process_results_for_failures(self):
        data = [{'response': Mock()}, {'exception': Mock()}, {'response': Mock()}]
        result = _process_results_for_failures(data)
        assert result == {'failure': 1, 'success': 2}
...
class WebhookProcessorRequestTestCase(TestCase):
    ...
    def test_send_message_success(self):
        with patch('webhook_processor.webhook_processor.post', return_value=self.mock_response(200)):
    ...
    def test_send_message_error(self):
        with patch('webhook_processor.webhook_processor.post', return_value=self.mock_response(500)):
            result = _send_message('http://www.example.com', None, None)
            response = result.get('response')
            assert response == None
            exception = result.get('exception')
            assert exception.message == '500 Server Error: None'
