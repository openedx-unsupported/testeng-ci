    ...
    def verify_upgrade_packages(self, pull_request):
        ...
        if txt:
            ...
            pull_request.create_issue_comment(
                ...
                f"{' '.join(self.make_readable_string(g) for g in valid_packages)}"
            )
            ...
            if not suspicious_pack and valid_packages:
                ...
                pull_request.create_issue_comment(
                    f"**`{len(suspicious_pack)}` Packages upgraded/downgraded with Major versions. "
                    f"PR needs manual review.**</br> \n "
